...
(defn pe044 [] (let [pentagonal-num (map #(- (* 3/2 % %) (* 1/2 %)) (iterate inc 1))
                     pentagonal? (fn [n] (zero? (rem (+ 1 (Math/sqrt (+ 1 (* 24 n)))) 6)))]
                 (long (first (for [j pentagonal-num
                                    k (take-while #(< % j) pentagonal-num)
                                    :let [diff (numeric/abs (- j k))]
                                    :when (pentagonal? diff)
                                    :when (pentagonal? (+ j k))]
                                diff)))))
